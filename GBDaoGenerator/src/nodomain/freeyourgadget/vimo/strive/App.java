/*
 * Modified By Murshid Hassen
 * Project Vimo-Strive
 * Main Class to pair Iot devices
 * SQLITE Library GreenDao Used as Gradle for Build Tool Specs
 * Referred in the Gradle as classpath 'org.greenrobot:greendao-gradle-plugin:3.2.2' // add plugin
 */
package nodomain.freeyourgadget.vimo.strive;
...
public class App {
    ...
    private static final String MAIN_PACKAGE = "nodomain.strive.vimo";  //Creating the Main Package "nodomain.freeyourgadget.vimo"
    ...
    public static void main(String[] args) throws Exception {
        ...
        Entity user = addUserInfo(schema, userAttributes);      // creating the user Entitty
    ...
    // this is your custom Device sample
    private static Entity addCustomDeviceSample(Schema schema, Entity user, Entity device) {
        Entity activitySample = addEntity(schema, "CustomActivitySample");
        activitySample.implementsSerializable();
        addCommonActivitySampleProperties("AbstractActivitySample", activitySample, user, device);
        activitySample.addIntProperty(SAMPLE_RAW_INTENSITY).notNull().codeBeforeGetterAndSetter(OVERRIDE);
        activitySample.addIntProperty(SAMPLE_STEPS).notNull().codeBeforeGetterAndSetter(OVERRIDE);
